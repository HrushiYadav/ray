...
kv-cache utilization, etc. [`RequestRouter`](/apis/serve/doc/ray.serve.request_router.RequestRouter.rst)
is an abstraction in Ray Serve that allows extension and customization of
load-balancing logic for each deployment.

This guide shows how to use [`RequestRouter`](/apis/serve/doc/ray.serve.request_router.RequestRouter.rst)
...
[`RequestRouter`](/apis/serve/doc/ray.serve.request_router.RequestRouter.rst). It implements the [`choose_replicas`](/apis/serve/doc/ray.serve.request_router.RequestRouter.choose_replicas.rst)
...
the replicas are able to process the request, [`choose_replicas`](/apis/serve/doc/ray.serve.request_router.RequestRouter.choose_replicas.rst)
will be called again with a backoff delay until a replica is able to process the
request.


:::{note}
This request router also implements [`on_request_routed`](/apis/serve/doc/ray.serve.request_router.RequestRouter.on_request_routed.rst)
...
the [`deployment`](/apis/serve/doc/ray.serve.deployment_decorator.rst)
...
- [`FIFOMixin`](/apis/serve/doc/ray.serve.request_router.FIFOMixin.rst): This mixin implements first in first out (FIFO)
  ...
  [`choose_replicas`](/apis/serve/doc/ray.serve.request_router.RequestRouter.choose_replicas.rst).
...
- [`LocalityMixin`](/apis/serve/doc/ray.serve.request_router.LocalityMixin.rst): This mixin implements locality-aware
  ...
  offers helpers [`apply_locality_routing`](/apis/serve/doc/ray.serve.request_router.LocalityMixin.apply_locality_routing.rst)
  and [`rank_replicas_via_locality`](/apis/serve/doc/ray.serve.request_router.LocalityMixin.rank_replicas_via_locality.rst) to route and
  ranks replicas based on their locality to the request, which can be useful for
  reducing latency and improving performance.
- [`MultiplexMixin`](/apis/serve/doc/ray.serve.request_router.MultiplexMixin.rst): When you use model-multiplexing
  ...
  helpers [`apply_multiplex_routing`](/apis/serve/doc/ray.serve.request_router.MultiplexMixin.apply_multiplex_routing.rst)
  and [`rank_replicas_via_multiplex`](/apis/serve/doc/ray.serve.request_router.MultiplexMixin.rank_replicas_via_multiplex.rst) to route
...
This request router inherits from [`RequestRouter`](/apis/serve/doc/ray.serve.request_router.RequestRouter.rst),
as well as [`FIFOMixin`](/apis/serve/doc/ray.serve.request_router.FIFOMixin.rst) for FIFO
request routing, [`LocalityMixin`](/apis/serve/doc/ray.serve.request_router.LocalityMixin.rst)
for locality-aware request routing, and
[`MultiplexMixin`](/apis/serve/doc/ray.serve.request_router.MultiplexMixin.rst)
for multiplexed model support. It implements
[`choose_replicas`](/apis/serve/doc/ray.serve.request_router.RequestRouter.choose_replicas.rst)
to take the highest ranked replicas from [`rank_replicas_via_multiplex`](/apis/serve/doc/ray.serve.request_router.MultiplexMixin.rank_replicas_via_multiplex.rst)
and [`rank_replicas_via_locality`](/apis/serve/doc/ray.serve.request_router.LocalityMixin.rank_replicas_via_locality.rst)
and uses the [`select_available_replicas`](/apis/serve/doc/ray.serve.request_router.RequestRouter.select_available_replicas.rst)
...
`request_router_class` argument of the [`deployment`](/apis/serve/doc/ray.serve.deployment_decorator.rst)
...
[`FIFOMixin`](/apis/serve/doc/ray.serve.request_router.FIFOMixin.rst) so queued
...
[`RequestRouterConfig`](/apis/serve/doc/ray.serve.config.RequestRouterConfig.rst):
...
[`request_router_kwargs`](/apis/serve/doc/ray.serve.config.RequestRouterConfig.rst):
...
3. A **deployment** that ties them together using a
   [deployment actor](/apis/serve/doc/ray.serve.config.DeploymentActorConfig.rst) for the queue and a
   [`RequestRouterConfig`](/apis/serve/doc/ray.serve.config.RequestRouterConfig.rst) for the router.
