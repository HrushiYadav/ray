...
[autoscaling_config](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) and an
...
#### **[`target_ongoing_requests`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [default=2]**
:::{note}
...
* **[`min_replicas`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [default=1]**: This is the minimum number of replicas for the
...
* **[`max_replicas`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [default=1]**: This is the maximum number of replicas for the
...
* **[`initial_replicas`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst)**: This is the number of replicas that are started
...
* **[`upscale_delay_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [default=30s]**: This defines how long Serve waits before
...
* **[`downscale_delay_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [default=600s]**: This defines how long Serve waits before
...
* **[`downscale_to_zero_delay_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [Optional]**: This defines how long Serve waits
...
* **[`upscale_smoothing_factor`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [default_value=1.0] (DEPRECATED)**: This parameter
is renamed to `upscaling_factor`. `upscale_smoothing_factor` will be removed in
a future release.

* **[`downscale_smoothing_factor`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [default_value=1.0] (DEPRECATED)**: This
parameter is renamed to `downscaling_factor`. `downscale_smoothing_factor` will
be removed in a future release.

* **[`upscaling_factor`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [default_value=1.0]**: The multiplicative factor to amplify
...
* **[`downscaling_factor`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [default_value=1.0]**: The multiplicative factor to
...
* **[`metrics_interval_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [default_value=10]**: In future this deployment level
...
* **[`look_back_period_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [default_value=30]**: This is the window over which the
average number of ongoing requests per replica is calculated.

* **[`aggregation_function`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) [default_value="mean"]**: This controls how metrics are
...
- **Frequency**: Every 10s (configurable via [`metrics_interval_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst))
...
- **Data sent**: Both raw timeseries data and pre-aggregated metrics
  - **Raw timeseries**: Data points are clipped to the [`look_back_period_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) window before sending (only recent measurements within the window are sent)
  - **Pre-aggregated metrics**: A simple average computed over the [`look_back_period_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) window at the replica/handle
...
- **Input**: Pre-aggregated simple averages from replicas/handles (already clipped to [`look_back_period_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst))
...
- **Input**: Raw timeseries data from replicas/handles (already clipped to [`look_back_period_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst))
- **Method**: Time-weighted aggregation using the [`aggregation_function`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) (mean, max, or min). Uses an instantaneous merge approach that treats metrics as right-continuous step functions.
- **Output**: Single value representing total ongoing requests
...
:::{note}
The [`aggregation_function`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) parameter only applies in aggregate mode. In simple mode, the aggregation is always a sum of the pre-computed simple averages.
:::
...
- **Input**: [`AutoscalingContext`](/apis/serve/doc/ray.serve.config.AutoscalingContext.rst)
...
- [`look_back_period_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) (30s) should be > push interval (10s)
...
`2 * `[`metrics_interval_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) to determine when to drop stale handle metrics.
...
* Lower the [`metrics_interval_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst).
Always set [`metrics_interval_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) to be less than
...
A custom autoscaling policy is a user-provided Python function that takes an [`AutoscalingContext`](/apis/serve/doc/ray.serve.config.AutoscalingContext.rst) and returns a tuple `(target_replicas, policy_state)` for a single Deployment.
...
Ray Serve automatically applies the following standard autoscaling parameters from your [`AutoscalingConfig`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) to custom policies:
...
You can make richer decisions by emitting your own metrics from the deployment. Implement `record_autoscaling_stats()` to return a `dict[str, float]`. Ray Serve will surface these values in the [`AutoscalingContext`](/apis/serve/doc/ray.serve.config.AutoscalingContext.rst).
...
* **`ctx.raw_metrics[metric_name]`** — A mapping of replica IDs to lists of raw metric values.
  The number of data points stored for each replica depends on the [`look_back_period_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) (the sliding window size) and [`metrics_interval_s`](/apis/serve/doc/ray.serve.config.AutoscalingConfig.rst) (the metric recording interval).
...
Ray Serve instantiates the class once on the controller when the deployment starts. `__init__` runs one-time setup, and `__call__` runs on every autoscaling tick with the current [`AutoscalingContext`](/apis/serve/doc/ray.serve.config.AutoscalingContext.rst).
...
- **Request body**: `{"target_num_replicas": <number>}` (must conform to the [`ScaleDeploymentRequest`](/apis/serve/doc/ray.serve.schema.ScaleDeploymentRequest.rst) schema)
